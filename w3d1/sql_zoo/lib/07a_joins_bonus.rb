...
def best_value
  ...
  execute(<<-SQL)
    ...
    HAVING
      albums.price / COUNT(tracks.song) <= 0.50
  SQL
end
...
def rock_superstars
  ...
  execute(<<-SQL)
    SELECT
      albums.artist, COUNT(albums.title)
    FROM
      albums
    WHERE
      albums.title IN (
...
def expensive_tastes
  ...
  execute(<<-SQL)
    SELECT
      styles.style, albums.price / COUNT(tracks.song) AS price_per_track
    FROM
      styles
    JOIN
      albums ON albums.asin = styles.album
    JOIN
      tracks ON styles.album = tracks.album
    GROUP BY
      styles.style, albums.title, albums.price
    ORDER BY
      albums.price / COUNT(tracks.song) DESC
    LIMIT
      5
